class OrdensModel {
  ...
  */
  ...
  async getForUpdateStatus({ filial, data_inicial_sinc_isat, ordens_com_servico }) {
    await this.db.query("SET client_encoding TO 'SQL_ASCII'");
    const result = await this.db.query(`
      SELECT ordem
      FROM ordem as a
      WHERE a.datasai>=current_date-60
        ...
        AND a.ordem>0
        ${!ordens_com_servico ? `AND (a.empresa='TODAS' or a.empresa='${filial}')` : ''}
        AND a.status<>'F'
  ...
  async treatCheck({ ordem, check, tot_cacambas }) {
    await this.db.query("SET client_encoding TO 'SQL_ASCII'");
  ...
  async setFinishOrder({ ordem }) {
    ...
    const result = await this.db.query(
      `UPDATE ordem SET status='F', obs='FINALIZADO', user_final='DRIVERSAT' WHERE ordem=${ordem}`
    );
  ...
  async retornoIsat({ ordem, situacao /*, movimenta_cacamba */ }) {
    ...
    await this.db.query(
      `UPDATE ordem SET retorno_isat='${situacao}' WHERE ordem=${ordem} AND retorno_isat<>'${situacao}'`
    );
